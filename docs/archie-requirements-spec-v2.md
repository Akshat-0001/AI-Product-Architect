# ARCHIE — Product Requirements Specification v2.0
...
> **ONE-LINE PITCH:** An AI Product Architect that turns your idea into a complete system design, documentation, and code — ready to build. Free forever, powered by your own API keys.

> **CORE PHILOSOPHY:** Open source. No paywalls. No usage limits. Users configure their own Groq or Gemini API keys in Settings and own their AI consumption entirely.
...
1. User arrives at `/` landing page → reads hero copy → clicks 'Start Building'
2. Redirected to `/auth` → signs up with GitHub or email
3. Redirected to `/settings/api-keys` → sees 'Set up your API keys to start generating' prompt
4. User goes to `console.groq.com`, gets free API key, pastes into Groq field → validates → green check
5. Clicks 'Start Building' → redirected to `/dashboard` → starts first project

### 5.2 Returning User (Key Already Configured)

1. Lands on `/dashboard` → sees existing projects + 'New Project' CTA
2. Clicks new project → Wizard Step 1 (idea input) → Step 2 (questions) → Step 3 (style) → generating → blueprint

### 5.3 Share Flow

1. User opens blueprint workspace → clicks 'Share'
2. Unique `/b/[shareId]` link generated → copied to clipboard
3. Anyone opens link → read-only blueprint view → 'Made with Archie' banner → converts to user
