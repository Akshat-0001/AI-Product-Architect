<!-- converted from archie-design-spec-v2.docx -->
...
## 1.4 Iconography
Icon library: Material Symbols Outlined (same as before). Key variation settings:
font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24 — default across all icons.
...
Width: 240px. Background: bg-slate-900 (approx #0f172a). Always visible in App Layout.
...
- Nav items (140px × 48px): icon + label. Active = indigo-400 text + border-l-4 border-indigo-500 + bg-slate-800/50
...
Height: 64-80px (varies: wizard = 64px with py-4, blueprint = 80px with h-20). Background: bg-slate-950/80 or bg-surface-container-low.
...
Fixed at bottom (z-50). Background: bg-surface-container-lowest/80 backdrop-blur-xl border-t border-outline-variant/10.
...
- Priority badges: High = bg-error/10 text-error, Medium = bg-tertiary/10 text-tertiary, Low = bg-outline-variant/20 text-outline. Status badges: Validated = primary-container/20 text-primary, Proposed = surface-container-highest text-outline.
...
- Code content: bg-surface-container-lowest/50 p-6, Fira Code sm leading-relaxed. Syntax colors: tertiary for import/keywords, secondary for booleans, primary-fixed for braces/delimiters, primary-fixed-dim for variables/functions, emerald-400 for strings, outline-variant for comments.
...
| GLASS PANEL FORMULA |
| --- |
| background: rgba(45, 52, 73, 0.6); backdrop-filter: blur(20px); border: 1px solid rgba(70, 69, 85, 0.15); — Applied to wizard cards, floating tooltips, terminal log, auth card |
| CTA GRADIENT |
| --- |
| background: linear-gradient(135deg, #adc6ff 0%, #005cc6 100%) — Used on all primary action buttons, FAB, wizard continue, generate button |
